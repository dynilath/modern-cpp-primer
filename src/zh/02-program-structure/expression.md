...
后缀自增运算符的作用是，将组成表达式的后缀表达式对象加一，表达式的值是这个对象加一之前的值。例如：
```cpp
int a = 10;
...
后缀自减运算符的作用是，将组成表达式的后缀表达式对象减一，表达式的值是这个对象减一之前的值。例如：
```cpp
int a = 10;
...
前缀自增运算符的作用是将组成表达式的后缀表达式对象加一，表达式的值是这个对象加一之后的值、且表示这个对象。例如：
```cpp
int a = 10;
...
前缀自减运算符的作用是将组成表达式的后缀表达式对象减一，表达式的值是这个对象减一之后的值、且表示这个对象。例如：
```cpp
int a = 10;
...
int a;
a = 42; // 可以对 a 进行赋值
int b = a; // 现在 a 的值是确定的，可以使用 a 进行计算了
```
...
在这一节中，已经提到了前缀自增、前缀自减、赋值表达式、复合赋值表达式等表达式的值是能指代对象的。这意味着下面的表达式是合法的：
```cpp
int a = 0;
int b = 0;
...
此外，如果 `()` 内的表达式是能指代对象的，那么这个基础表达式也指代这个对象的。例如：
```cpp
(a) = 42; // 合法
```
