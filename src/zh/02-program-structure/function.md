...
```
返回值类型名 函数名(参数列表) 复合语句
```
其中，`返回值类型名` 表示函数的返回值类型，是一个表示类型的描述符；`函数名` 表示函数的名称，是一个描述符；`复合语句` 表示函数的函数体。

返回值是函数计算的结果，`返回值类型名` 表示返回值的类型。

函数名是一个用来代指函数的标识符，函数名可以用来调用函数。注意，函数并不是一个对象，你不能使用赋值表达式来给函数赋值。

参数列表可以为空，或者是一个用逗号分隔的参数声明列表，形式是：
- ① `参数声明`
- ② `参数声明列表 , 参数声明`
每个参数声明的形式是 `类型 标识符` 或者 `类型`（省略标识符）。
...
```
上面的代码里，`int` 是返回值类型名，`sqrt` 是函数名，`int x` 是参数列表。

## 返回语句

在上面这段函数的最后，出现了一个 `return a;` ，这是一种**跳转语句**，称为**返回语句**。

`return` 语句的形式是 `return 表达式;`，在执行 `return` 语句时，会计算 `表达式` 的值，并将这个值作为函数的返回值，然后结束函数的执行。

这里，`return a;` 的作用是将 `a` 的值作为函数的返回值，然后结束函数的执行。

## 函数的调用

现在，我们要介绍如何调用函数。函数的调用是一种[后缀表达式](./expression.md#后缀表达式)，其形式是：
```
函数名 ( 参数列表 ) 
```
...
有的函数不需要返回值，这样的函数称为**无返回值函数**。无返回值函数的返回值类型名是 `void`。例如，我们可以定义一个无返回值函数 `add_a`：
```cpp
int a = 1;
...
无返回值函数的返回值类型是 `void`，这是一个特殊的类型，表示没有返回值。因此，无返回值函数的 `return` 语句是没有表达式的，例如：
```cpp
int a = 1;
void add_a(int x) {
    if(x > 0) return; // 无返回值
...
```
在上面的程序里，`add_sqrt_counter` 函数在 `sqrt` 函数中被调用，但是 `add_sqrt_counter` 函数的定义在 `sqrt` 函数的定义之后。这样的程序是错误的。然而，`sqrt_counter` 
 的初始化需要调用 `sqrt` 函数，`add_sqrt_counter` 函数的定义又需要 `sqrt_counter` 的初始化。这样的循环依赖是无法解决的。

这时，我们可以使用**前向声明**。函数的前向声明的形式是：
```
返回值类型名 函数名(参数列表);
```
目前，你可以将函数的前向声明的形式理解为把函数体改成一个分号 `;`。
