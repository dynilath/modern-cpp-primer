...
**表达式语句**简单来说就是一个[表达式](./expression/brief.md)后面跟着一个分号 `;`。例如：
...
在[表达式](./expression/sup-content2.md)中，介绍了声明语句的形式是：`类型 初始化声明, 初始化声明, 初始化声明 ... ;`。
...
这里 `label` 是一个标号，`statement` 是一个语句。标号的结尾总是一个冒号 `:`，标号后面的语句可以是任意一种语句。
...
::: tip
有时候，我们需要将一个[相等表达式](./expression/rel-expr.md#相等表达式)作为 `condition`，例如：
```cpp
if (a == b) equal = true;
```
初学者会经常错误地将 `a == b` 写成 `a = b`。但注意 `a = b` 是一个[赋值表达式](./expression/assign-expr.md)，它将 `b` 赋值给 `a`，并且返回 `a` 的值。这样，并不会做 `a` 和 `b` 是否相等的判断。
:::
