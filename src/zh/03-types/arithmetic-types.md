...
基础算数类型是对平台硬件所支持的指令的一种抽象。虽然在程序语言上会有各种各样的扩展类型，但是它们都必须作为硬件支持的形式来处理。

在常见的平台上，通过算数逻辑单元处理整数类型，通过浮点单元处理浮点类型。这一现实在 C++ 语言上体现为浮点和整数仿佛是两个冤家，在一个浮点数加入了聚会之后，整个聚会就被迫变成了浮点数的聚会，这时候那些位运算就被请出了门。如果非要让浮点数加入整数的聚会，我们就不得不费劲地把浮点数塞进整数的衣服里。

基础整数类型之中，`int` 应当是最快的整数类型（或之一）。同时，算数逻辑单元常常并不接受任何大小的数据，而是至少要接受 `int` 尺寸的数据，并且参与运算的两个数据也要求大小相同。正因如此，一般算数转换中会将小于 `int` 的整数类型转换为 `int`，并且必须找到一个公共的类型来进行运算。

事实上，这种先抽象成基础类型，再抽象成扩展类型的二次抽象是一种过于复杂的历史遗留。程序员仍然需要浪费心智查看硬件手册，以理解 `int` 和 `char` 究竟是什么东西。已经进行了尝试的读者也会发现，几乎所有情况下，`std::int_fast8_t` 就是 `signed char`，`std::int_fast32_t` 就是 `int`，`std::int_fast64_t` 就是 `long int`，这层抽象在实际中几乎没有产生任何价值。于是，更现代的设计直接将扩展类型作为基础类型，具体如何调度算数逻辑单元让编译器来分析。笔者也推荐读者直接使用扩展类型，而不要费神纠结基础类型的性质。
:::
