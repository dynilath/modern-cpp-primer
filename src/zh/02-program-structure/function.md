...
**返回值**是函数计算的结果，**返回值类型**表示返回值的类型（返回值在后面[返回语句](#返回语句)中介绍）。
...
```
上面的代码里，`int` 是返回值类型，`sqrt` 是函数名，`int x` 是参数列表。
...
有的函数不需要返回值，这样的函数称为**无返回值函数**。无返回值函数的返回值类型是 `void`。例如，我们可以定义一个无返回值函数 `add_a`：
```cpp
int a = 1;
