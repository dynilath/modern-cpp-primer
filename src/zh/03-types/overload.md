...
考虑如下的几个函数（读者如果已经忘记了函数的形式参数的名字是可以省略的，可以回顾一下[前面的章节](../02-program-structure/function.md)）。
...
这些函数有完全相同的名字，当写下如下的函数调用的时候，会发生什么？
```cpp
sqrt(42);
```
...
这种根据参数类型从选择**同名函数**中选择最合适的一个调用的特性，称为**重载**。 

## 重载的作用

重载通常用于实现相似功能的函数。前面的例子中，如果不使用重载，这种需求就会变成如下的形式：
...
这会导致代码越多，调用层级越多，修改的工作量就会几何倍地增加。最终导致代码维护成本难以控制。
