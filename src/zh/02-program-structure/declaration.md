...
::: info 抽象用语的内存
在这里，“内存”并不是具体的指你的个人电脑的内存条提供的RAM存储器，而是一个抽象机的描述用语。例如，在寄存器上的一个字节，对于 C++ 而言也是内存里的一个字节，实现可以根据对象的使用情况编译为需要的具体调度形式。在高等编程语言中，这样的抽象将语言和硬件实现分离，使得语言可以适用于不同的硬件平台，程序员也无需阅读硬件手册来编写程序。
:::

::: info 对象与变量
很多时候，人们会用“变量”来描述类似“对象”的概念。技术性的说，“变量”是指除了非静态成员的引用声明以及对象声明引入的实体，也即是说，变量不一定是对象。这里“非静态成员”、“引用”、“实体”等概念在后面的章节中会介绍，目前读者不需要提前了解。
...
**不确定值**的默认初始化规则对于用户自定义类型会有所不同，技术性地说，这里的不确定值规则仅限内置类型的默认初始化。对于更复杂类型的默认初始化，我们会在后面的章节中介绍。
:::
