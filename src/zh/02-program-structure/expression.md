...
:::info
技术性的来说，一元表达式形式应当是 `+ 转换表达式`、`- 转换表达式`、`后缀表达式`等。`后缀表达式` 又由`函数调用`、`数组访问`、`基本表达式`等组成。
...
+'a' // 值为 97
```
这个转换被称为整数提升，类型大小比 `int` 小的类型在正运算符、负运算符的表达式中会被提升为 `int` 类型。
...
逻辑非运算符的作用是对里面的基本表达式取逻辑非，也就是说，逻辑非的结果一定是 `bool` 类型。例如：
```cpp
!42 // 值为 0
```
`42` 被转换为 `true`，然后取逻辑非，所以结果是 `false`。

## 加性表达式和乘性表达式

加性表达式和乘性表达式组成了 C++ 中的算数表达式。
...
:::info
技术性的来说，乘性表达式的形式中的①是 `乘性表达式 * 成员指针表达式`，②是 `乘性表达式 / 成员指针表达式`，以此类推。
...
a + b - c
```
这里 C++ 规定从左往右依次计算，即先计算 `a + b`，然后再计算 `a + b - c`。
...
技术性的来说，对于任意 `op` 运算符，`a op= b` 等价于 `a = a op b`。除了这里的 `a` 表达式只计算一次，其他的计算过程和赋值表达式是一样的。
